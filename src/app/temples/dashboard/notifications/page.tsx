...
export default function TempleNotificationsPage() {
    const [notifications, setNotifications] = useState<Notification[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState("");

    const [templeAdminUser, setTempleAdminUser] = useState<any>(null);

    useEffect(() => {
        const userStr = localStorage.getItem("temple_user");
        if (userStr) {
            setTempleAdminUser(JSON.parse(userStr));
        }
    }, []);

    const userId = templeAdminUser?.id || templeAdminUser?.email;
    const userType = 'temple_admin';
    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Temple Notifications</h1>
                    <p className="text-muted-foreground">
                        Stay updated on temple activity, bookings, and alerts.
                    </p>
                </div>
                {unreadCount > 0 && (
